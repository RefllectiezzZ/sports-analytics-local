"""Test-only Betano synthetic fixture parser (``betano-fixture-bundle-v1``).

Production acquisition must use :mod:`sports_analytics.sources.betano.native_parser`.
"""
...
from sports_analytics.sources.betano.catalog import PARSER_VERSION
...
SYNTHETIC_SCHEMA = "betano-fixture-bundle-v1"


def parse_betano_synthetic_payloads(
    ...
    *,
    ...
    """Parse one or more sanitized Betano synthetic fixture payloads (tests only)."""
    ...
    return ProviderAcquisitionBundle(
        ...
        provenance=tuple(sorted(set(provenance) | {f"parser:{PARSER_VERSION}:synthetic"})),
    )
...
def _parse_market(raw: dict[str, Any]) -> ProviderMarketObservation:
    ...
    for item in raw.get("selections", []):
        if not isinstance(item, dict):
            continue
        sel_id = str(item.get("source_selection_id", ""))
        if sel_id in seen:
