"""Production Betclic native offering parser (``betclic-offering-v1``)."""
...
from sports_analytics.sources.betclic.catalog import PARSER_VERSION, PROVIDER_ID
from sports_analytics.sources.betclic.native_mappings import (
    BETCLIC_NATIVE_SCHEMA,
...
def parse_betclic_acquisition(
    ...
    *,
    ...
    if acquisition.provider_id != PROVIDER_ID:
        msg = f"betclic parser received provider {acquisition.provider_id}"
    ...
    return parse_betclic_native_payloads(
        payloads,
        provider_id=PROVIDER_ID,
        adapter_version=adapter_version,
        acquisition_cycle_id=acquisition.acquisition_cycle_id,
...
def parse_betclic_native_payloads(
    payloads: list[dict[str, Any]],
    *,
    provider_id: str,
    adapter_version: str,
    ...
    """Parse one or more provider-native Betclic offering payloads."""
    ...
    for payload in payloads:
        ...
        if schema == "betclic-fixture-bundle-v1":
            ...
            continue
        if schema != BETCLIC_NATIVE_SCHEMA:
        ...
        for raw_match in payload.get("matches", []):
            if not isinstance(raw_match, dict):
                ...
                continue
            try:
                events.append(_parse_native_match(raw_match, default_sport=sport))
...
def _parse_native_match(raw: dict[str, Any], *, default_sport: str) -> ProviderEventObservation:
    sport_code = str(raw.get("sport") or default_sport)
    sport = map_betclic_sport_code(sport_code, default_sport=default_sport)
    state_raw = str(raw.get("matchState") or "PREMATCH").upper()
    state = _BETCLIC_EVENT_STATE.get(state_raw)
    if state is None:
        msg = f"unsupported match state {state_raw!r}"
        raise ParserError(msg)
    ...
    participants = tuple(
        ProviderParticipantObservation(
            source_participant_id=str(item["contestantId"]),
            display_name=str(item["name"]),
            role=BETCLIC_PARTICIPANT_ROLE_MAPPINGS.get(
                str(item.get("side", "")).upper(),
                str(item.get("side", "unknown")).lower(),
            ),
            normalized_name=(
...
def _parse_native_market(raw: dict[str, Any]) -> ProviderMarketObservation:
    type_code = str(raw.get("marketType") or "")
    canonical = map_betclic_market_type(type_code)
    status_raw = str(raw.get("status") or "OPEN").upper()
    market_status = _BETCLIC_MARKET_STATUS.get(status_raw, MarketStatus.OPEN)
    period_code = raw.get("period")
    period = (
        BETCLIC_PERIOD_MAPPINGS.get(str(period_code).upper()) if period_code is not None else None
    )
    ...
    return ProviderMarketObservation(
        ...
        display_label=str(raw.get("label", type_code)),
