...
def build_canonical_participant_key(
    *,
    ...
    """Build the source-independent canonical participant key."""
    ...
    return f"participant|{sport_code}|{participant_type}|{canonical_key}"
...
def build_source_participant_key(
    *,
    ...
    return f"{source_name}|{sport_code}|{normalized_name}"
...
def build_canonical_event_key(
    *,
    ...
    The key deliberately excludes ``source_name`` so two sources describing the
    same fixture derive the same canonical event identity.
    """
    validate_domain_identifier(sport_code, field_name="sport_code")
    ...
    if home_canonical_participant_id == away_canonical_participant_id:
        ...
        raise NormalizationError(msg)
    return (
        f"event|{sport_code}|{competition_id}|{season_id}|{event_date.isoformat()}|"
