"""Offline Betclic fixture-bundle parser tests."""
...
FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "betclic"
...
def _parse(payload: dict[str, Any], *, sport: str = "football") -> ProviderAcquisitionBundle:
    return parse_betclic_synthetic_payloads(
        ...
        acquisition_cycle_id="cycle-betclic-test",
...
def test_parses_football_fixture_with_locale_decimals() -> None:
    ...
    assert odds["betclic-sel-home"] == Decimal("1.85")
    assert odds["betclic-sel-draw"] == Decimal("3.40")
    assert odds["betclic-sel-away"] == Decimal("4.20")
...
def test_missing_odds_preserves_typed_unpriced_selection() -> None:
    ...
    assert selections["betclic-sel-home"].decimal_odds is None
    assert selections["betclic-sel-home"].price_state is ProviderSelectionPriceState.UNPRICED
    assert all(
        selection.price_state is ProviderSelectionPriceState.PRICED
        for source_id, selection in selections.items()
        if source_id != "betclic-sel-home"
    )
...
def test_duplicate_selection_identities_reject_event() -> None:
    ...
    market["selections"].append(
        {
            "source_selection_id": "betclic-sel-home",
...
def test_unknown_markets_retained_for_audit() -> None:
    ...
    payload["events"][0]["markets"].append(
        {
            "source_market_id": "betclic-mkt-unknown",
            ...
            "selections": [
                {
                    "source_selection_id": "betclic-sel-unk",
    ...
    acquisition = BrowserAcquisitionResult(
        ...
        responses=(
            BrowserResponseObservation(
                provider_id=PROVIDER_ID,
                page_route_id="football-prematch",
                response_url="https://www.betclic.pt/api/synthetic",
                observed_at_utc=OBSERVED_AT,
